...
const d = reactive({
  ...
  command: "start",
  isAgree: uGetLoginInfo("slAgrRequired"),
  isCerti: uGetLoginInfo("identVerRequired"),
...
const onClickNext = () => {
  ...
  if (d.age14 > 13 && isEqual(d.isCerti, "Y")) {
    if (!d.checkMob) {
      modal.alert("알림", "본인인증이 필요합니다.");
      return;
    }
  }
  ...
  if (d.age14 > 18 && isEqual(login.joinInfo.join_pcd, "JOIN_P002")) {
    router.push("/revalidation/selectUniv");
  } else if (isEqual(d.isSchool, "Y") || isEqual(d.isLearn, "Y")) {
...
const saveCertification = async () => {
  await call({
    method: "post",
    id: "서울런 약관동의 및 본인인증",
    endpoint: "/api/platform_chg/termsAgrAndIdent",
    data: {
      nice_enc_dat: login.passInfo.nice_enc_dat,
...
const onCahngeTerms = () => {
  console.log("onCahngeTerms", d.termsList);
  if (d.age14 < 14 && d.termsList.every((s) => s.value === true)) {
    d.checkMob = true;
  } else if (
    isEqual(d.isAgree, "Y") &&
    d.termsList.every((s) => s.value === true)
  ) {
    d.checkMob = true;
  }
};
...
<template>
  ...
  <!-- 이메일 인증 팝업 -->
  <div class="account">
    <h1>{{ uGetSubtitle(2) }}</h1>
    <Nav :active="2" />
    <div class="account__body">
      <div class="join" v-if="d.age14 < 14">
        <h2 class="account__title">약관동의 및 본인인증</h2>
        <template v-if="isEqual(d.isAgree, 'Y')">
          <div class="account__agree">
            <div class="account__agree-title">‘서울런’ 약관동의</div>
            <TermsGroup
              v-model="d.termsList"
              @terms="joinTerms"
              @change="onCahngeTerms"
            />
          </div>
        </template>
        <!-- <template v-if="isEqual(d.isCerti, 'Y')">
          <div class="c-form-group">
            ...
            <div class="account__info">
              ...
              <div class="account__info-title c-text-danger">
                ...
                <span>
                  만 만 14세 미만 아동의 개인 정보 보호를 위해 [개인정보 보호법]
